...
CREATE TABLE IF NOT EXISTS coworking.reservations (
    ...
    status VARCHAR(255) NOT NULL CHECK (status IN ('PENDING_PAYMENT', 'CONFIRMED', 'CANCELLED', 'COMPLETED')),
...
CREATE INDEX IF NOT EXISTS idx_reservations_space_date ON coworking.reservations(space_id, date);
CREATE INDEX IF NOT EXISTS idx_reservations_user ON coworking.reservations(user_id);
CREATE INDEX IF NOT EXISTS idx_reservations_status ON coworking.reservations(status);
